...
class Users_adminController extends Controller
{
    ...
    */
	public function getIndex(Request $request)
	{
		...
		if ($request->input('keywords')){
			$users = DB::table('shop_users_admin')->where('username','like','%'.$request->input('keywords').'%')->paginate($num);
		}else{
			$users = DB::table('shop_users_admin')->paginate($num);
		}
		$list = $request->all();
		//解析一个模板
		return view('admin.users_admin.index',['users'=>$users,'list'=>$list]);
	}
	...
	*/
	...
	{
		//解析一个 模板
		return  view('admin.users_admin.add');
	}
	...
	*/
	public function postInsert(Request $request)
	{
		...
		// dd($data);
		//发送语句
		$res = DB::table('shop_users_admin')->insert($data);
		// 判断是不是成功了
		if ($res) {
			return redirect('admin/users_admin')->with('success','用户添加成功');
		}else{
	...
	*/
	public function getDelete(Request $request)
	{
		...
		// dd($id);
		//删除这个用户	
		$res = DB::table('shop_users_admin')->where('uid',$id)->delete();
	...
	*/
	public function postUpdate(Request $request)
	{
		...
		$res = DB::table('shop_users_admin')->where('uid',$id)->update($data);

		if($res){
			//成功到列表页
			return redirect('/admin/users_admin/index')->with('success','用户修改成功');
		}else{
			//失败后回滚
